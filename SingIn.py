...
os.system("mkdir ./sing_in_image/" + OpenTime)
times_of_find=0
...
while True:
  ...
  if flag == "see":
      ...
      for names,times,number in zip( name_of_sing_in,time_of_sing_in,range( len(name_of_sing_in) ) ):
          print("{0: ^4}".format(str(number+1)))
          print("{0: ^10}".format(str(names)))
          print("{0: ^5}".format("已在" + times))
          print("{0: ^5}\n".format("签到"))
      print("{0:=^36}".format("今日未签到："))
      for names in name_of_unsing_in:
          print("{0: ^32}".format(names))
      continue
  if NotFirstPrint == 0:
  ...
  if name == "Unknown":
      ...
      with open('sing_in.txt', 'a') as f: #签到信息保存至sing_in.txt中
           f.write("  " + name + "  已在  " + time_str + "  完成签到\n") 
      # cv2.imwrite("./sing_in_image/"+ OpenTime +"/" + name + time_str +".jpg",frame)
